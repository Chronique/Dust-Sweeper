...
import { Sun, Moon, Share2, Github, HelpCircle } from "lucide-react";
                ...
                >
                  {(() => {
                    ...
                    if (chain.unsupported) {
                      return (
                        <button onClick={openChainModal} type="button" className="flex items-center gap-2 px-4 py-2 bg-red-600 text-white rounded-full text-xs font-bold hover:bg-red-500 shadow-md transition-all active:scale-95">
                          Wrong Network
                        </button>
                      );
                    }
                    return (
                      ...
                      >
                        {userPfp ? (
                          <Image src={userPfp} alt="Profile" width={36} height={36} className="w-9 h-9 rounded-full border-2 border-green-500 object-cover" priority />
                        ) : (
