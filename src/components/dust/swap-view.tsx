...
interface SwapViewProps {
  ...
  onTabChange?: (tab: TabType) => void; // ← new: to navigate to Earn tab
}
...
export const SwapView = ({ defaultFromToken, onTokenConsumed, onTabChange }: SwapViewProps) => {
  ...
  // ── NEW: post-sweep YO CTA state ──────────────────────────────────────────
  const [showYoCta, setShowYoCta]         = useState(false);
  const [sweptWethDisplay, setSweptWethDisplay] = useState("");
  ...
  const selectAll = () => {
    ...
    else { setSelectedTokens(new Set(tokens.slice(0, MAX_BATCH_TOKENS).map(t => t.contractAddress))); }
    setSimulation(null);
  ...
  const handleWithdrawToken = async (token: TokenData) => {
    if (!walletClient) return;
    const amount = await prompt(`Enter amount to withdraw:`, token.formattedBal, { title: `Withdraw ${token.symbol}`, placeholder: "e.g. 1.5", confirmText: "Withdraw" });
    if (!amount || isNaN(Number(amount)) || Number(amount) <= 0) return;
    const ok = await confirm(`Withdraw ${amount} ${token.symbol} to your wallet?`, { title: "Confirm Withdrawal", confirmText: "Withdraw" });
    if (!ok) return;
    try {
  ...
  const handleSimulate = async () => {
    ...
    try {
      ...
      const tokensToSwap = tokens
        .filter(t => selectedTokens.has(t.contractAddress) && t.contractAddress.toLowerCase() !== WETH_ADDRESS.toLowerCase())
        .slice(0, MAX_BATCH_TOKENS)
        .map(t => ({ address: t.contractAddress as Address, symbol: t.symbol, balance: BigInt(t.rawBalance) }));
      if (tokensToSwap.length === 0) { setToast({ msg: "No swappable tokens selected (WETH excluded).", type: "error" }); return; }
      const result = await simulateBatchSwap({ walletClient, chainId: chainId ?? 8453, tokens: tokensToSwap });
  ...
  const handleExecute = async () => {
    ...
    try {
      ...
      const tokensToExecute = simulation.processable.map(c => ({ address: c.token, symbol: c.symbol, balance: c.balance }));
      const execResult = await executeBatchSwap({ walletClient, chainId: chainId ?? 8453, tokens: tokensToExecute });
      console.log("[Swap] Batch tx confirmed ✓", execResult.txHash);

      const skippedCount  = simulation.skipped.length;
      const wethOut       = parseFloat(formatEther(simulation.totalNetWeth)).toFixed(5);
      const tokenCount    = simulation.processable.length;

      // Show success toast
      setToast({
        msg:  `✓ ${tokenCount} token${tokenCount > 1 ? "s" : ""} swept → ${wethOut} WETH${skippedCount > 0 ? ` (${skippedCount} skipped)` : ""}`,
        type: "success",
      });

      // Show Save to YO CTA
      setSweptWethDisplay(wethOut);
      setShowYoCta(true);
    ...
    } finally { setExecuting(false); setExecProgress(""); }
  };
  ...
  return (
    <div className="pb-36 space-y-4">
      ...
      {/* ── Save to YO CTA — shown after successful sweep ── */}
      {showYoCta && (
        <div className="rounded-2xl border-2 border-yellow-400/60 bg-gradient-to-br from-yellow-50 to-amber-50 dark:from-yellow-900/30 dark:to-amber-900/20 p-4 animate-in slide-in-from-top-3 duration-400">
          <div className="flex items-start gap-3">
            <div className="w-9 h-9 rounded-xl bg-yellow-400/20 flex items-center justify-center shrink-0 mt-0.5">
              <Sprout className="w-5 h-5 text-yellow-600 dark:text-yellow-400" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-yellow-800 dark:text-yellow-200">
                {sweptWethDisplay} WETH ready to earn yield
              </p>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                Swap WETH → USDC and save to YO Protocol to start earning now.
              </p>
            </div>
            <button onClick={() => setShowYoCta(false)} className="text-zinc-400 hover:text-zinc-600 text-xs shrink-0 mt-1">✕</button>
          </div>
          <div className="flex gap-2 mt-3">
            <button
              onClick={() => { setShowYoCta(false); onTabChange?.("tanam"); }}
              className="flex-1 py-2.5 rounded-xl font-bold text-sm bg-yellow-400 hover:bg-yellow-500 text-yellow-900 transition-colors flex items-center justify-center gap-1.5"
            >
              <Sprout className="w-4 h-4" /> Save to YO →
            </button>
            <button
              onClick={() => setShowYoCta(false)}
              className="px-4 py-2.5 rounded-xl text-sm text-zinc-500 bg-zinc-100 dark:bg-zinc-800 hover:bg-zinc-200 dark:hover:bg-zinc-700 transition-colors"
            >
              Later
            </button>
          </div>
      ...
      {incomingToken && (
        <div className="flex items-center gap-3 p-3 rounded-xl bg-orange-500/10 border border-orange-500/30 animate-in slide-in-from-top-2 duration-300">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-sm text-orange-100 truncate">{incomingToken.symbol} — {parseFloat(incomingToken.formattedBal).toFixed(4)}</div>
          </div>
      ...
      <div className="bg-gradient-to-t from-green-900 to-red-900 border border-red-800/40 rounded-2xl p-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-green-200 mt-1">Token → WETH (then save to YO or unwrap to ETH)</p>
        ...
        <div className="rounded-2xl border border-zinc-700 bg-zinc-900 p-4 space-y-3 animate-in slide-in-from-top-2 duration-200">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="bg-zinc-800 rounded-xl p-3">
              ...
              <div className={`text-base font-bold ${simulation.skipped.length > 0 ? "text-red-400" : "text-zinc-400"}`}>{simulation.skipped.length} tokens</div>
            </div>
            <div className="bg-green-900/30 border border-green-700/40 rounded-xl p-3">
              ...
              <div className="text-base font-bold text-green-300 font-mono">{parseFloat(formatEther(simulation.totalNetWeth)).toFixed(6)} WETH</div>
            </div>
            <div className="bg-zinc-800 rounded-xl p-3">
              <div className="text-[10px] text-zinc-500 mb-0.5">Platform Fee (5%)</div>
              <div className="text-base font-bold text-zinc-300 font-mono">{parseFloat(formatEther(simulation.totalFee)).toFixed(6)} WETH</div>
            </div>
          </div>
              ...
              {simulation.processable.map((item, i) => (
                <div key={i} className="flex items-center justify-between text-xs bg-zinc-800/50 rounded-lg px-2.5 py-1.5">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-green-400 font-mono">{item.netWethOut > 0n ? `~${parseFloat(formatEther(item.netWethOut)).toFixed(5)} WETH` : "unwrap"}</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          {simulation.skipped.length > 0 && (
            <div className="space-y-1">
              <div className="text-[10px] text-red-500 uppercase font-bold px-1 flex items-center gap-1"><WarningTriangle className="w-3 h-3" /> Skipped</div>
          ...
          {simulation.processable.length > 0 && (
            ...
            >
              {executing
                ...
                : <><Flash className="w-4 h-4" />Sweep {simulation.processable.length} Token{simulation.processable.length > 1 ? "s" : ""} → WETH · 1 tx</>}
            </button>
          )}
          <p className="text-[10px] text-zinc-500 text-center">WETH can be saved to YO Protocol or unwrapped to ETH in the Earn tab</p>
        </div>
      )}
      ...
      <div className="flex items-center justify-between px-2">
        <div className="text-sm font-bold text-zinc-500">
          ...
          {selectedTokens.size > 0 && <span className="ml-2 text-blue-400">{selectedTokens.size}/{MAX_BATCH_TOKENS} selected</span>}
        </div>
        <div className="flex items-center gap-2">
          ...
          <button onClick={selectAll} className="text-xs font-medium text-blue-500 hover:text-blue-400">
            {selectedTokens.size === Math.min(tokens.length, MAX_BATCH_TOKENS) && tokens.length > 0 ? "Deselect All" : `Select Top ${MAX_BATCH_TOKENS}`}
          </button>
        </div>
      </div>
            ...
            return (
              <div
                ...
                className={`flex items-center justify-between p-3 rounded-xl border transition-all ${
                  isDisabled ? "opacity-40 cursor-not-allowed bg-zinc-900 border-zinc-800"
                  : "cursor-pointer " + (isSelected
                    ? isIncoming ? "bg-orange-900/20 border-orange-500/50" : "bg-blue-900/20 border-blue-500/50"
                    : "bg-white dark:bg-zinc-900 border-zinc-100 dark:border-zinc-800 hover:border-zinc-700")
                }`}
              >
                <div className="flex items-center gap-3">
                  <div className={`w-5 h-5 rounded-full flex items-center justify-center border ${
                    isSelected ? isIncoming ? "bg-orange-500 border-orange-500 text-white" : "bg-blue-500 border-blue-500 text-white" : "border-zinc-600 text-transparent"
                  }`}>
      ...
      {noRouteTokens.length > 0 && (
        <div className="space-y-2 mt-6">
          ...
          <div className="space-y-2">
            {noRouteTokens.slice((vaultPage - 1) * VAULT_PER_PAGE, vaultPage * VAULT_PER_PAGE).map((token, i) => (
              <div key={i} className="flex items-center justify-between p-3 rounded-xl border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-zinc-900">
                ...
                <button onClick={() => handleWithdrawToken(token)} className="px-2.5 py-1.5 text-xs font-medium rounded-lg bg-blue-50 text-blue-600 hover:bg-blue-100 dark:bg-blue-900/20 dark:text-blue-400">WD</button>
              </div>
            ))}
          </div>
          {totalNoRoutePgs > 1 && (
            <div className="flex justify-center items-center gap-1 mt-2">
              ...
              {Array.from({ length: totalNoRoutePgs }, (_, i) => i + 1)
                ...
                .map((p, i) => p === "..." ? <span key={i} className="px-2 text-zinc-400 text-xs">...</span>
        ...
        <div className="fixed bottom-24 left-4 right-4 z-40 animate-in slide-in-from-bottom-5">
          ...
          >
            {simulating
              ...
              : <><Flash className="w-5 h-5 text-yellow-400" />Preview Sweep ({selectedTokens.size} token{selectedTokens.size > 1 ? "s" : ""})</>}
          </button>
          <div className="text-center text-[10px] text-zinc-400 mt-2">Estimate WETH & fee · max {MAX_BATCH_TOKENS} tokens · 1 confirmation</div>
        </div>
      )}
    </div>
  );
