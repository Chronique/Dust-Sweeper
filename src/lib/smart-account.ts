...
export const getSmartAccountClient = async (walletClient: WalletClient) => {
  ...
  return createSmartAccountClient({
    ...
    // MIDDLEWARE INI KUNCI UTAMANYA!
    middleware: {
      gasPrice: async () => {
        try {
          // Plan A: Pimlico
          const gasPrice = await bundlerClient.getUserOperationGasPrice();
          if (gasPrice && gasPrice.fast) return gasPrice.fast;
          throw new Error("Pimlico undefined");
        } catch (error) {
          // Plan B: Fallback ke Base RPC (Mengatasi error undefined)
          console.warn("⚠️ Pakai Fallback Gas Base");
